...
let mockDataDir = path.join(process.cwd(), './mock')
if (fs.existsSync(path.join(__dirname, '../bin/tpl'))) {

    mockDataDir = __dirname
}


app.use("/index.html", (req, res) => {
    let data = JSON.parse(cat(path.join(mockDataDir, './data/index.json')))
    let apiList = JSON.parse(cat(path.join(mockDataDir, './data/0.json')))
...
app.use("/___api/cate/add", (req, res) => {
    ...
    fs.writeFile(path.join(mockDataDir, './data/index.json'), JSON.stringify(data), 'utf8', (err, rst) => {
        if (err) {
            console.error(path + ' is not fond');
            process.exitCode = 1;
            return;
        }
        fs.writeFile(path.join(mockDataDir, './data/' + (data.cate.length - 1) + '.json'), '{}', 'utf8', (err, rst) => {
            if (err) {
                console.error(path + ' is not fond');
                process.exitCode = 1;
                return;
...
app.use("/___api/cate/get", (req, res) => {
    let {id} = req.body;
    let dataJson = JSON.parse(cat(path.join(mockDataDir, `./data/${id}.json`))),
        dataArray = []
...
app.use("/___api/list/add", (req, res) => {
    ...
    fs.writeFile(path.join(mockDataDir, `./data/${id}.json`), JSON.stringify(dataJson), 'utf8', (err, rst) => {
        if (err) {
            console.error(path + ' is not fond');
            process.exitCode = 1;
            return;
...
app.use("/*", (req, res) => {
    ...
    let data = JSON.parse(cat(path.join(mockDataDir, './data/index.json')));
    let apiUrl = originalUrl.replace(/\//g, ':') + sliptor + method.toLowerCase()
    let pathApi = data.list[apiUrl], pathApiAttr = apiUrl
    if (!pathApi) {
        for (let attr in data.list) {
            if (!pathApi && new RegExp('^' + attr + '$').test(apiUrl)) {
                pathApi = data.list[attr];
                pathApiAttr = attr
            }
        }
        if (!pathApi) {
    ...
    let dataJson = JSON.parse(cat(path.join(mockDataDir, pathApi)))
    res.end(JSON.stringify({
        code: '0',
        data: Mock.mock(getMockData(dataJson[pathApiAttr].list)),
        msg: ""
    }))
});
