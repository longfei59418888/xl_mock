...
app.use("/___api/list/add", (req, res) => {
    ...
    let {apiName = '', apiUrl = '', apiDesp = '', apiMethod = '', list = [], cateId = '', id = ''} = reqBody;
    if (apiUrl.substr(0, 1) != '/') apiUrl = '/' + apiUrl
    let saveData = {apiName, apiDesp, apiMethod, list, cateId, id, apiUrl}
...
app.use("/*", (req, res) => {
    ...
    res.end(JSON.stringify({
        code: '0',
        data: Mock.mock(getMockData(currentData.list)),
        msg: ""
    }))
});
